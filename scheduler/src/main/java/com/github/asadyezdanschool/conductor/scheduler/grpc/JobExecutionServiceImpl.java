...
public class JobExecutionServiceImpl extends JobExecutionServiceGrpc.JobExecutionServiceImplBase {
    ...
    public void getHttpRunDetails(GetHttpRunDetailsRequest request,
        ...
        try {
            ...
            GetHttpRunDetailsResponse.Builder builder = GetHttpRunDetailsResponse.newBuilder()
            ...
            if (details.headers() != null) {
                for (com.github.asadyezdanschool.conductor.scheduler.model.HttpHeader h : details.headers()) {
                    builder.addHeaders(HttpHeader.newBuilder()
                            .setName(h.name()).setValue(h.value()).build());
                }
            }
            GetHttpRunDetailsResponse response = builder.build();
